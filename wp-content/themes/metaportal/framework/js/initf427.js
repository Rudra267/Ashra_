...
//			var holdable			= $('#social,.metaportal_fn_search');
		...
		filterAjaxCall: function(element,page,action,translate){
			...
			var collection = $('.metaportal_fn_nfts').data('collection').toString();
			if(collection !== '' && !MetaPortalFilterArray.length){
				if(collection.indexOf(',') > -1){
					MetaPortalFilterArray = collection.split(',');
				}else{
					MetaPortalFilterArray = [collection];
				}				
			}
			...
			$.ajax({
				...
				success: function(data) {
					var index = MetaPortalFilterArray.indexOf(collection);
					if(index !== -1){
						MetaPortalFilterArray.splice(index, 1);
					}
